...
import { Activity, Shield, Zap, TrendingUp, Compass, Target, Calendar, BarChart2, Lightbulb, Grid, Settings, RefreshCw, CheckCircle, XCircle, ThumbsUp, ThumbsDown, Radio, Cpu, Award } from 'lucide-react';
export default function App() {
    ...
    const handleRateComment = async (oppId, commentIndex, rating) => {
    ...
    const handleAddUserIdea = async (e) => {
        e.preventDefault();
    ...
    const handleAddAnalytics = async (e) => {
        e.preventDefault();
    ...
    return (<div className="min-h-screen bg-black text-cyan-400 font-mono flex flex-col selection:bg-cyan-500 selection:text-black">
      {/* Top Header Bar */}
      <header className="border-b border-cyan-500/30 bg-black/80 backdrop-blur-md sticky top-0 z-50 px-6 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center space-x-6 text-sm">
          ...
          <button onClick={fetchAllData} className="flex items-center space-x-1 px-3 py-1 rounded border border-cyan-500/40 bg-cyan-950/30 hover:bg-cyan-900/40 transition text-cyan-300 text-xs">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="w-64 border-r border-cyan-500/30 bg-black/90 p-4 flex flex-col space-y-1 overflow-y-auto">
          {[
            ...
            return (<button key={item.id} onClick={() => setActiveTab(item.id)} className={`flex items-center space-x-3 px-3 py-2.5 rounded text-sm transition text-left ${isActive
        ...
        <main className="flex-1 overflow-y-auto p-6 bg-gradient-to-br from-black via-cyan-950/10 to-black relative">
          ...
          {/* ==================== TAB: DASHBOARD ==================== */}
          {activeTab === 'dashboard' && (<div className="space-y-6 relative z-10">
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-center">
                ...
                <div className="space-y-4">
                  <div className="jarvis-card p-4 rounded-lg">
                    ...
                    <div className="space-y-3">
                      {dashboardData?.topTrends?.map((trend) => (<div key={trend.id} className="p-2.5 rounded bg-cyan-950/30 border border-cyan-500/30 text-xs">
              ...
              <div className="jarvis-card p-6 rounded-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {dashboardData?.todayIdeas?.map((idea, idx) => (<div key={idea.id} className="p-4 rounded border border-cyan-500/40 bg-black/60 flex flex-col justify-between">
                      <div>
                        <div className="flex items-center justify-between text-xs mb-2">
                          <span className="text-cyan-500">IDEA #{idx + 1} // {idea.contentType}</span>
             // {idea.contentType}</span>
                      ...
                      <div className="flex items-center justify-between pt-3 border-t border-cyan-500/20 text-xs">
                        ...
                        <button onClick={() => setActiveTab('daily-ideas')} className="text-cyan-300 hover:underline">
                          View Details →
                        </button>
                      </div>
                    </div>))}
          ...
          {/* ==================== TAB: DAILY IDEAS ==================== */}
          {activeTab === 'daily-ideas' && (<div className="space-y-6 relative z-10">
              ...
              <div className="grid grid-cols-1 gap-6">
                {dailyIdeas.map((idea, idx) => (<div key={idea.id} className="jarvis-card p-6 rounded-lg space-y-4">
                    <div className="flex items-start justify-between">
                      ...
                      <div className="flex items-center space-x-2">
                        <button onClick={() => handleIdeaStatus(idea.id, 'APPROVED')} className={`px-3 py-1.5 rounded text-xs border flex items-center space-x-1 ${idea.status === 'APPROVED' ? 'bg-cyan-500 text-black border-cyan-400 font-bold' : 'bg-cyan-950/40 border-cyan-500/40 text-cyan-300 hover:bg-cyan-900/40'}`}>
                          <CheckCircle className="w-3.5 h-3.5"/>
                          <span>Approve</span>
                        </button>
                        <button onClick={() => handleIdeaStatus(idea.id, 'REJECTED')} className={`px-3 py-1.5 rounded text-xs border flex items-center space-x-1 ${idea.status === 'REJECTED' ? 'bg-red-950 text-red-300 border-red-500 font-bold' : 'bg-red-950/20 border-red-500/30 text-red-400 hover:bg-red-900/40'}`}>
          ...
          {/* ==================== TAB: TRENDS RADAR ==================== */}
          {activeTab === 'trends' && (<div className="space-y-6 relative z-10">
              <div className="flex items-center justify-between">
                <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {trends.map((trend) => (<div key={trend.id} className="jarvis-card p-4 rounded-lg flex flex-col justify-between space-y-3">
                    <div>
                      <div className="flex items-center justify-between text-xs mb-2">
          ...
          {activeTab === 'brand-comment' && (<div className="space-y-6 relative z-10">
              <div className="flex items-center justify-between">
                <div>
              ...
              <div className="jarvis-card p-6 rounded-lg flex flex-col items-center justify-center relative overflow-hidden min-h-[300px]">
                ...
                return (<div key={opp.id} style={pos} className="absolute w-3 h-3 bg-red-400 rounded-full animate-ping cursor-pointer" title={opp.title}></div>);
            })}
              </div>

              {/* Opportunities List with Suggested Comments */}
              <div className="grid grid-cols-1 gap-6">
                {brandOpportunities.map((opp) => (<div key={opp.id} className="jarvis-card p-6 rounded-lg space-y-4">
                    <div className="flex items-start justify-between">
                      <div>
                        <div className="text-xs text-cyan-500 mb-1">PLATFORM: {opp.platform.toUpperCase()} // VIRAL OPPORTUNITY</div>
             // VIRAL OPPORTUNITY</div>
                        <h3 className="text-lg font-bold text-cyan-200">{opp.title}</h3>
                        <p className="text-xs text-cyan-400/80 mt-1">{opp.description}</p>
                      </div>
                      <a href={opp.videoUrl} target="_blank" rel="noreferrer" className="px-3 py-1.5 rounded bg-cyan-950 border border-cyan-500/40 text-xs text-cyan-300 hover:bg-cyan-900/40 transition">
                        Open Video ↗
                      </a>
                    </div>
                    ...
                    {opp.brandAccounts && opp.brandAccounts.length > 0 && (<div className="space-y-2">
                        <span className="text-xs text-cyan-600 uppercase">Brands already commenting:</span>
                        <div className="space-y-1">
                          {opp.brandAccounts.map((brand, bIdx) => (<div key={bIdx} className="text-xs bg-cyan-950/20 p-2 rounded border border-cyan-500/20 flex justify-between">
                    ...
                    <div className="space-y-2 pt-3 border-t border-cyan-500/20">
                      ...
                      <div className="space-y-2">
                        {opp.suggestedComments.map((sug, sIdx) => (<div key={sIdx} className="text-xs bg-black/70 p-3 rounded border border-cyan-500/40 flex items-center justify-between">
                            <div>
                            ...
                            <div className="flex items-center space-x-2">
                              <button onClick={() => handleRateComment(opp.id, sIdx, true)} className={`p-1 rounded border ${sug.rating === true ? 'bg-cyan-500 text-black border-cyan-400' : 'border-cyan-500/30 text-cyan-400 hover:bg-cyan-950'}`}>
                                <ThumbsUp className="w-3.5 h-3.5"/>
                              </button>
                              <button onClick={() => handleRateComment(opp.id, sIdx, false)} className={`p-1 rounded border ${sug.rating === false ? 'bg-red-500 text-black border-red-400' : 'border-cyan-500/30 text-red-400 hover:bg-red-950'}`}>
          ...
          {/* ==================== TAB: COMPETITORS ==================== */}
          {activeTab === 'competitors' && (<div className="space-y-6 relative z-10">
              <div className="flex items-center justify-between">
                <div>
              ...
              <form onSubmit={handleAddCompetitor} className="jarvis-card p-4 rounded-lg flex gap-4 items-end">
                <div className="flex-1">
                  ...
                  <input type="text" placeholder="@ryanair" value={newCompetitor.username} onChange={e => setNewCompetitor({ ...newCompetitor, username: e.target.value })} className="w-full bg-black border border-cyan-500/40 rounded px-3 py-2 text-xs text-cyan-300 focus:outline-none focus:border-cyan-400"/>
                </div>
                <div className="w-40">
                  <label className="text-xs text-cyan-600 block mb-1">Platform</label>
                  <select value={newCompetitor.platform} onChange={e => setNewCompetitor({ ...newCompetitor, platform: e.target.value })} className="w-full bg-black border border-cyan-500/40 rounded px-3 py-2 text-xs text-cyan-300 focus:outline-none focus:border-cyan-400">
              ...
              {/* Competitors Grid */}
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {competitors.map((comp) => (<div key={comp.id} className="jarvis-card p-4 rounded-lg space-y-3">
                    <div className="flex items-center justify-between">
                    ...
                    <div className="flex justify-between items-center text-xs pt-2 border-t border-cyan-500/20">
                      ...
                      {!comp.isApproved && (<button onClick={async () => {
          ...
          {/* ==================== TAB: MY IDEAS ==================== */}
          {activeTab === 'my-ideas' && (<div className="space-y-6 relative z-10">
              <div>
              ...
              <form onSubmit={handleAddUserIdea} className="jarvis-card p-4 rounded-lg space-y-3">
                ...
                <textarea rows={3} placeholder="e.g. Tiger dancing to a random Roblox trending sound..." value={newIdeaInput} onChange={e => setNewIdeaInput(e.target.value)} className="w-full bg-black border border-cyan-500/40 rounded p-3 text-xs text-cyan-300 focus:outline-none focus:border-cyan-400"></textarea>
              ...
              <div className="grid grid-cols-1 gap-4">
                {myIdeas.map((idea) => (<div key={idea.id} className="jarvis-card p-6 rounded-lg space-y-3">
          ...
          {/* ==================== TAB: CALENDAR ==================== */}
          {activeTab === 'calendar' && (<div className="space-y-6 relative z-10">
              <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {calendar.map((item) => (<div key={item.id} className="jarvis-card p-4 rounded-lg space-y-2">
                ...
                {calendar.length === 0 && (<div className="jarvis-card p-6 rounded-lg text-center text-cyan-500 text-xs">
          ...
          {/* ==================== TAB: ANALYTICS ==================== */}
          {activeTab === 'analytics' && (<div className="space-y-6 relative z-10">
              <div>
              ...
              <form onSubmit={handleAddAnalytics} className="jarvis-card p-4 rounded-lg space-y-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div>
                    ...
                    <input type="number" value={manualAnalytics.views} onChange={e => setManualAnalytics({ ...manualAnalytics, views: e.target.value })} className="w-full bg-black border border-cyan-500/40 rounded px-3 py-2 text-xs text-cyan-300"/>
                  </div>
                  <div>
                    <label className="text-xs text-cyan-600 block mb-1">Likes</label>
                    <input type="number" value={manualAnalytics.likes} onChange={e => setManualAnalytics({ ...manualAnalytics, likes: e.target.value })} className="w-full bg-black border border-cyan-500/40 rounded px-3 py-2 text-xs text-cyan-300"/>
                  </div>
                  <div>
                    <label className="text-xs text-cyan-600 block mb-1">Comments</label>
                    <input type="number" value={manualAnalytics.comments} onChange={e => setManualAnalytics({ ...manualAnalytics, comments: e.target.value })} className="w-full bg-black border border-cyan-500/40 rounded px-3 py-2 text-xs text-cyan-300"/>
                  </div>
                  <div>
                    <label className="text-xs text-cyan-600 block mb-1">Shares</label>
                    <input type="number" value={manualAnalytics.shares} onChange={e => setManualAnalytics({ ...manualAnalytics, shares: e.target.value })} className="w-full bg-black border border-cyan-500/40 rounded px-3 py-2 text-xs text-cyan-300"/>
                  </div>
                </div>
                <button type="submit" className="px-4 py-2 bg-cyan-950 border border-cyan-500 text-cyan-300 text-xs rounded hover:bg-cyan-900 transition">
              ...
              <div className="grid grid-cols-1 gap-4">
                {analytics.map((item) => (<div key={item.id} className="jarvis-card p-4 rounded-lg flex items-center justify-between text-xs">
                    <div>
                      <span className="text-cyan-500">Post ID: {item.postId} // {new Date(item.postedAt).toLocaleDateString()}</span>
             // {new Date(item.postedAt).toLocaleDateString()}</span>
                    </div>
          ...
          {/* ==================== TAB: WHAT WORKS ==================== */}
          {activeTab === 'what-works' && (<div className="space-y-6 relative z-10">
              <div>
              ...
              <div className="grid grid-cols-1 gap-4">
                {whatWorks.map((item) => (<div key={item.id} className="jarvis-card p-6 rounded-lg space-y-2">
                    <div className="flex justify-between items-center text-xs text-cyan-500">
          ...
          {/* ==================== TAB: CATEGORIES ==================== */}
          {activeTab === 'categories' && (<div className="space-y-6 relative z-10">
              <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {categories.map((cat) => (<div key={cat.id} className="jarvis-card p-6 rounded-lg space-y-3">
                    ...
                    {!cat.isApproved && (<button onClick={async () => {
          ...
          {/* ==================== TAB: SETTINGS ==================== */}
          {activeTab === 'settings' && (<div className="space-y-6 relative z-10 max-w-2xl">
              <div>
              ...
              <div className="jarvis-card p-6 rounded-lg space-y-4 text-xs">
                <div>
                  <label className="text-cyan-500 uppercase block mb-1">OpenRouter AI Model</label>
                  <input type="text" value="openrouter/free (Default Cost-Effective)" disabled className="w-full bg-black border border-cyan-500/40 rounded px-3 py-2 text-cyan-400 cursor-not-allowed"/>
                </div>
                <div>
                ...
                <div>
                  ...
                  <div className="flex items-center justify-between bg-black/50 p-3 rounded border border-cyan-500/30">
                    ...
                    <button onClick={() => alert('Push notifications enabled successfully!')} className="px-3 py-1 bg-cyan-500 text-black font-bold rounded hover:bg-cyan-400 transition">
                      Enable Push
                    </button>
                  </div>
                </div>
